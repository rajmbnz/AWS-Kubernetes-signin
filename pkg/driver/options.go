...
type Options struct {
	...
	// LegacyXFSProgs enables the use of `` when formatting a volume with XFS. This is necessary if the driver is used
	// to mount XFS volumes on Linux Nodes with a kernel version ≥ 5.4 due to __.
	LegacyXFSProgs bool
}
...
func (o *Options) AddFlags(f *flag.FlagSet) {
	...
	if o.Mode == AllMode || o.Mode == NodeMode {
		...
		f.BoolVar(&o.LegacyXFSProgs, "legacy-xfs", false, "PLACEHOLDER")
	}
}
